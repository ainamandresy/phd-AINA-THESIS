...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Some sections of this chapter is part of the paper [G. Sadaka, A. Rakotondrandisa, F. Luddens, C. Lothodé, P-H. Tournier, I. Danaila, \textit{Parallel finite-element codes for the simulation of solid-liquid phase-change systems with natural convection}, to be submitted to CPC, 2019].

We now perform parallel computation of three-dimensional liquid-solid phase-change systems involving natural convection in this chapter.
...
Our motivation to expand our numerical model to 3D configuration is first motivated by the lack of publications in the literature for accurate 3D simulations of phase-change materials using adaptive grids.
...
Since the mesh adaptivity procedure is done through a sequential algorithm, we compute the metrics with respect to the coarse mesh, then finer local meshes are generated in parallel during the mesh decomposition step in order to reach the desired level of refinement for the subdomains.
...
In Sec. \ref{sec: natconv-air-3D}, we first validate the 3D natural convection of air in a cubic cavity against numerical benchmark by \cite{Wakashima-2004},
and compare together the solutions obtained using the parallel or the sequential algorithms.
Second, the natural convection of water is investigated in Sec. \ref{sec-3D-water-convec}. %to illustrates the capability of our method to deal with non-linear body force $f_B$ and to capture accurately interfaces, mainly the density inversion.
Then, the lateral melting of n-octadecane PCM inside 3D enclosure is addressed in Sec. \ref{sec-3D_melting}.
We finally present the challenging numerical simulation of water freezing in Sec. \ref{sec-3D_Water_FREEZING}.
...
The physical parameters are the same than used in the 2D simulations ($\Prd = 0.71$) and we investigate Rayleigh numbers ranging from $\Ray=10^4$ to $\Ray=10^6$. 
...
\label{fig-3DT} 
\end{figure}
We first compare the current simulation with the numerical data of \cite{Wakashima-2004}, who used a forth order finite difference method, with a vorticity-stream function formulation with different uniform meshes of  $120 \times 120 \times 120 \times 10$ grid nodes.
...
The higher the Rayleigh number, the finer the thermal boundary layer thickness in the vicinity of the vertical walls.
   ...
   \begin{center}
      \begin{tabular}{*{3}{cl}}
           ...
           \cite{Raluca2013} & $0.0830$ ($0.8550$) & $0.2553$ ($0.03905$) \\ \bottomrule       
               ...
               \end{tabular}
   \end{center}
   \caption{Natural convection of air in a differentially heated cavity using $3-$D configurations. Maximum values $u_{max}$ and $v_{max}$ of the velocity profiles at mid-domain ($x = 0.5$ and $y=0.5$, respectively) and their locations $y_{max}$ and $x_{max}$. Comparison with benchmark solutions of \cite{Wakashima-2004} and numerical data of \cite{Raluca2013} for $\Ray = 10^4$, $\Ray = 10^5$, $\Ray = 10^6$.}
   \label{tab-3D-locU}
\end{table}
...
The number of subdomain varies from $28$ to $70$ for $1.8$ millions of unknowns to assess the consistency of our method with the number of subdomains.
...
\begin{table}[!h]
	\begin{center}
		...
		\end{tabular}
	\end{center}
	\caption {Natural convection of air in a differentially heated cavity using $3-$D configurations. Comparison between sequential and ffddm algorithm for uniform grids of $40 \times 40 \times 40$ }
	\label{tab-T1}
\end{table}
...
We simulate the natural convection of water inside cubic cavity using adaptive $3-$D meshes in this section.
...
The temperature distribution and the corresponding adapted mesh, at the steady state is shown in Fig. \ref{fig-3D_Water_convec}a. 
...
Smaller tetrahedra are clearly observed in the vicinity the walls, where a spiral movement of the fluid occurs along the walls whereas boundary layer heat transfer takes place in the same region, and between the two counter-rotating circulation patterns.
...
Furthermore, the authors have noted that, even though the temperature profiles at the central cross-section of the cavity match well for both $2-$ and $3-$D solutions in their numerical simulations, serious differences between two and three-dimensional velocity profiles are always present. %at some points.
...
Triangular symbols denote the reference solution, obtained with 3D finite difference code using vorticity-vector potential formulation of the Navier-Stokes and energy equations.
...
We now proceed to the $3-$D simulation of solid-liquid phase-change problem, starting with the melting of PCM in a cube-shape domain.
The three-dimensionality of the fluid flow can lead to major differences between the two-dimensional solution in some configurations, at the example of the melting of Gallium.
...
The three-dimensional simulations exhibit the absence of a multi-vortex structure at any time step.
They have explained these differences  by the presence of walls in the third direction which suppress the flow and by the occurrence of weak turbulence in the bulk of the melt preventing the formation of the long-living multicellular flow pattern known from previous 2D simulations.
The differences between $2$D and $3$D flows are that, the three-dimensional model allows a fluid movement in all directions, and the friction effect provided by no-slip walls in width direction influences the flow.
\cite{wittig2012three} have highlighted the existence of a secondary flow in the width direction, as shown in Fig.  \ref{fig-3D_PCM_MELTING}, which has maximal intensity near side walls.

We expand the validated $2$D simulations in Chapter \ref{chap-MELTING} to $3$D configurations and solutions obtained by both configurations are compared to assess the three-dimensional effects that occurs during the melting process.
...
The melt front is defined by the flow which rises along the hot wall and moves down along the solid-liquid interface, where it is cooled.
The influence of the secondary flow, defined by  \cite{wittig2012three}, is apparent with respect to the curved shape of the interface in the vicinity of the side-walls in the width direction.
...
\end{equation}
In our simulations, at $t = 78.7$, we obtained $U_{xyz} = 0.548208$ for $3$D simulations and $U_{xyz} = 0.519576$ for $2$D.
However, the time evolution of the heat transfer in Panel (b) match well for $2-$ and $3-$D simulations, with a maximum difference lower than $1 \%$ at the onset of the convection time and overlaps during all the simulation time.
From an engineering point of view, the latter observation means that if only an assessment of the heat transfer during the phase-change process is sought, a $2$D simulation is enough since it could be performed in realistic computational time.
The $3$D simulation lasts $2$ days with $200$ processors, while $45$ minutes on personal computer is spent for $2$D configuration, for the investigated Rayleigh number.
...
A last $3$D simulation of the challenging case of ice formation inside differentially heated cubic cavity is considered in this section.
...
The temperature distribution and the adapted mesh at final time $t_{\varphi} = 2340$ s ($t = 1.61$) are displayed at Figs. \ref{fig-3D_Water_Freeze}a and \ref{fig-3D_Water_Freeze}b.
...
For visual identification of the flow structures during the ice formation, Panel (c) plots the snapshots of velocity vectors, iso-surface $\theta = 0.4$ along the anomalous density variation, and phase-change front at time instant $t = 1.61$.
%The shape of the iso-surface $\theta = 0.4$, along the anomalous density variation, and phase-change front are depicted in panel (c) with the velocity field in the liquid flow.
The effect of the secondary flow, discussed in Sec. \ref{sec-3D_melting}, is also visible by the curved shape of the iso-surface $\theta = \theta_m$ in the width $y-$ direction.
One can note however, that the shape of the solidification front is almost $2$D.
The buoyancy-induced fluid motion in the abnormal recirculation region is too weak to influence the solid front. Finally, the overlay of the experimental image of \cite{Kowalewski-1999} with the current simulation exhibits good agreement for the location of the solid-liquid interface.
...
\end{minipage}
\end{center}
\caption{Freezing of pure water in 3D cubical cavity.  Temperature distribution and adapted mesh at (physical time) $t_{\varphi}=2340 [s]$ (t = 1.61) (a). Zoom of the adapted mesh at the mid-plane (b). Temperature iso-surfaces $\theta=0.4$ and $\theta = 0$ with the velocity vectors in the liquid phase (c). Superimposition of the interface between experimental image and the present simulation (red solid line) (d).}
\label{fig-3D_Water_Freeze} 
\end{figure}

\section{Concluding remarks}
In this chapter, parallel $3-$D tools to simulate natural convection flow involving melting or solidification boundary condition, using domain decomposition method has been performed.
The numerical simulations investigated with the recent library \texttt{ffddm} in \ff, showed good agreement with the benchmark solutions or the sequential algorithm for all simulated cases.
The qualitative behavior and the morphology of the interface, for phase-change problems, were discussed in detail.
The three-dimensional effects of the flow were exhibited by the deformed shape of the interface in the width direction.
%While the three-dimensionality of the flow influence the phase-change front, the time evolution of the heat transfer remains the same for both configurations, for the investigated Rayleigh numbers.
The adaptive $3-$D mesh procedure proved very efficient to capture several interfaces, at the example of the water freezing case.
Grid refinement effort was prescribed only in regions displaying high gradients of the computed variables, and $10$ larger tetrahedra were applied elsewhere.
