...
%\sectionmark{head version}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\def\titcourt{A finite element method}
\def\titlong{A finite element method for the Navier-Stokes-Boussinesq equations using a Newton algorithm}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
finite difference (FD) or finite volume (FV) methods are used most of the time with a fixed-mesh approach.
Accurate simulations are therefore carried out by refining considerably the mesh resolution in the whole domain, and thus increasing dramatically the computational time.
In the example of \cite{voller1996cyclic} who used insufficient grid resolution to compute the melting of Gallium, \cite{wang2010numerical} had to consider thinner meshes, resulting in a strong increase, of a factor of three, of the total mesh nodes to capture correctly the boundary layer structure. 

Our choice for the finite element (FE) method is motivated by its capability to adapt dynamically the mesh and to deal with several geometrical domains.
The adaptive capabilities of the FE discretization by using finer meshes where sharp phenomena takes place (solid-liquid interfaces, boundary layers, recirculation zones) and coarser meshes elsewhere (in the solid, in the bulk of the fluid region where the gradient are lower than the boundary layer region) is helpful to reduce the degrees of freedom involved in the numerical resolution and thus reduce the computational time.
We use a finite-element method that was implemented using the open-source software FreeFem++ \citep{freefem,hecht-2012-JNM}.

\noindent The use of mesh adaptivity proved mandatory in this study to obtain accurate results within reasonable computational time.
...
\cite{aldbaissy2018full} have compared analytically and numerically the first and second order discretization of the time-dependent Boussinesq problem, and have concluded that the second order scheme is better than the first order in terms of CPU time when the same precision is required.
...
\noindent Some authors used a Richardson extrapolation \citep{Belhamadia2012,wang2010numerical} in the momentum equation by extrapolating U from previous time steps.
We have tested this approach, but the results exhibited less accurate solutions and the computations requested small $\delta t$.
In addition, a projection algorithm with an explicit discretization of the Navier-Stokes-Boussinesq equations was also investigated, using the Adams-Bashforth and Crank-Nicolson second order schemes.
The main drawback is the very small $\delta t$ ($\sim 10^{-6}$) needed to ensure convergence at each time steps.
A last alternative we tested for the treatment of non-linear terms in the momentum equation is the characteristics Galerkin method \citep{Pironneau92}, but this method is not convenient for high $\Ray$ simulations and requires very small time steps.

Finally, a supplementary difficulty comes from the one-domain method which needs techniques to bring the velocity to zero in the solid region.
...
We note that the physical meaning of the variable viscosity formulation and the Carman-Kozeny penalty term is fundamentally different.
...
The viscosity-based method was investigated by \cite{dan-2014-JCP} and the Carman-Kozeny penalty method is investigated in the present work.
We note however that techniques in FV methods based on the modification of the numerical algorithm to switch-off the velocity in the solid by a relaxation scheme also exists.

\newpage
\section{Finite element algorithm} \label{sec-FE-algo}

To solve the system of Eqs. \eqref{eq-qmvt} - \eqref{eq-energ} we use a finite-element method. %that was implemented using the open-source software \ff \citep{freefem,hecht-2012-JNM}.
...
We use the second-order implicit finite-difference discretization in Eq. \eqref{eq-Gear} of the temporal derivative and obtain the time semi-discretization of the single-domain model \eqref{eq-qmvt} - \eqref{eq-energ}:
...
The space discretization of variables over the domain $\Omega$ uses a finite-element method based on a weak formulation of the system of Eqs. \eqref{eq-time-disc1} - \eqref{eq-time-disc3}. 
...
\noindent Following the generalization of the Stokes problem \citep{Temam,GRaviart,Quarteroni}, the variational formulation of the system  of Eqs. \eqref{eq-time-disc1} - \eqref{eq-time-disc3} can be written as: find $(\vec{u}^{n+1}, p^{n+1}, \theta^{n+1}) \in \vec{V}\times Q\times V$, such that
\begin{eqnarray}
\label{eq-weak-all}
...
\noindent where {$(u , v)=\int_{\Omega} u\cdot v$} denotes the scalar product in $L^2(\Omega)$ or $\left(L^2(\Omega)\right)^2$; the bilinear forms $a, b$ and trilinear form $c$ are defined as \citep{GRaviart,Quarteroni}:
...
The system of non-linear Eqs. \eqref{eq-weak-all} - \eqref{eq-weak-energy}, regarded as a general equation ${\cal F}(w) = 0$, with $w = (u,p,\theta)$, is solved using a Newton method. To advance the solution from time $t_n$ to $t_{n+1}$, we start from an initial guess $w_0 = (\vec{u}^{n}, p^{n}, \theta^{n})$ (which is the solution at $t_n$), and construct the Newton sequence $w_k = (u_k, p_k, \theta_k)$ by solving for each inner iteration $k$:
...
Eq. \eqref{eq-newton-w} can be rewritten as follows:
...
\noindent Note that the last term of Eq. \eqref{eq-newton-C2} cancels in the case of a linear Boussinesq force $f_B$  (see Eq. \eqref{eq-RePr}); this is not the case when non-linear variations of the density of the liquid are considered (convection or solidification of water).  Note also that the previous system of Eqs. \eqref{eq-newton-C1} - \eqref{eq-newton-C3} depends only on $\vec{u}^n$, $\vec{u}^{n-1}$, $\theta^n$ and $\theta^{n-1}$ and is independent of $p^n$, the pressure being in this approach a Lagrange multiplier for the divergence free constraint. 
...
&solve Eq. \eqref{eq-newton-C1} to get $ \vec w_{k+1} $\\
...
\noindent In some cases, it is more relevant to solve directly the steady equation corresponding to Eqs. \eqref{eq-weak-all} - \eqref{eq-weak-energy} (i.e. without the temporal derivatives), when a steady state could be achieved.
It is for example the case for natural convection of air or water in differentially heated configurations and low $\Ray$ numbers.
In the stationary case, the weak formulation of the problem becomes:
...
\noindent The parameter $\alpha$ is introduced in front of the buoyancy force term $f_B$ as a continuation parameter of the value of the $\Ray$ number.
...
The idea is to solve successively steady problems by increasing $\alpha$ from $0$ to $1$.
At each stage, the previously computed solution is set as initial condition.
These steps could be assimilated to a smooth increase of the $\Ray$ number,

\begin{equation} \label{eq-Newton-steady}
\begin{tabular}{||ll}
\multicolumn{2}{||l}{Loops for $m = 1: \mathcal{N}$ steps}\\
\multicolumn{2}{||l}{set  $w_0=(\vec{u}^{n}, p^{n}, \theta^{n})$}\\
& \begin{tabular}[t]{||ll}
\multicolumn{2}{||l}{Newton iterations  following $k$}\\
&solve Eq. \eqref{eq-weak-steady} - \eqref{eq-weak-energy-steady} with $\alpha = \left( \frac{m}{\mathcal{N}} \right)^4$\\
\multicolumn{2}{||l}{stop when  $\| \vec w_{k+1} - \vec w_k \| < \xi_N$}
\end{tabular}\\
\multicolumn{2}{||l}{actualize $(\vec{u}^{n+1}, p^{n+1}, \theta^{n+1}) = w_{k+1}$}
\end{tabular}
...
\noindent with $\mathcal{N}$ the number of steps of the continuation algorithm.

\noindent This approach allows to compute the steady natural convection of air in $6$ iterations for $\Ray = 10^6$ instead of $150$ iterations when the unsteady system of equations is solved until a steady state is reached.
...
\begin{itemize}
   ...
   \item \textbf{ scalAdim}: defines the characteristic scales of the problem, see Eq. \eqref{eq-adim}. Possible values 1, 2 or 3 correspond to the following choice of the characteristic scales \citep{dan-2014-JCP}:
...
\end{eqnarray}
   ...
   \item \textbf{ Pr, Ra, Ste}: are the  Prandtl, Rayleigh and Stefan numbers, see Eq. \eqref{eq-Rayleigh} and \eqref{eq-RePr},
   \item \textbf{ T$_{hot}$, T$_{cold}$}: are  dimensionless temperatures according to Eq. \eqref{eq-adim},
 ...
 The parameters of the hyperbolic-tangent function in Eq. \eqref{eq-smooth}, used to regularize discontinuous functions are set by default as follows:
 \end{itemize}
    \begin{table}[!ht]
    \centering
    \begin{tabular}{*{8}{c}}
...
\begin{itemize}
   ...
   \item \textbf{ gamma}: is the penalty parameter in Eq. \eqref{eq-time-disc1}. \underline{Default value} = $10^{-7}$,
   \item \textbf{ tolNewton}: is the Newton tolerance $\xi_N$ (see Eq. \eqref{eq-Newton-algo}). \underline{Default value} = $10^{-6}$,
   \item \textbf{ newtonMax}: limits the maximum number of iterations in  the Newton algorithm \eqref{eq-Newton-algo}. \\
...
We tested the space and time accuracy using manufactured solutions for the system of Eqs. \eqref{eq-qmvt} - \eqref{eq-energ} for a stationary case (Burggraf flow) and a time-dependent one \citep{nourgaliev2016fully}. For both cases, we computed the global error $ \varepsilon$ for different norms in space:
\begin{equation}
...
The forcing terms that have to be added to the momentum and energy (temperature) equation are derived by injecting the exact solution \eqref{burggraf-u} into the system of Eqs. \eqref{eq-qmvt} - \eqref{eq-energ}:
\begin{eqnarray}
   f_{u_1} &=& 0, \\ \nonumber
...
Global error $\varepsilon$ \eqref{eq-epsconv} for  the temperature: (c) $\PP_1$ and (d) $\PP_2$ finite elements.}
...
For diminishing values of the time step $\delta t$, the solution was evolved in time up to the time instant $t_{max} = \pi$ at which the error Eq. \eqref{eq-epsconv} was computed. 
...
\begin{figure}[!ht]
	...
	\end{center}
	\caption{Time accuracy of the numerical scheme tested using the time-dependent manufactured solution of \cite{nourgaliev2016fully}. Evolution of the global error $\varepsilon$ \eqref{eq-epsconv} for  the temperature at $t_{max} = \pi$. Discretizations using: (a) $\PP_1$ and (b) $\PP_2$finite elements.}
	\label{fig-conv-bdf2}
\end{figure}

\newpage
\section{Domain decomposition method with FreeFem++: FFDDM}
Solving the Navier-Stokes-Boussinesq systems of Eqs. \eqref{eq-weak-all} - \eqref{eq-weak-energy} in three-dimensional configurations could generate a large problem size.
The natural convection of air in a cube of dimensions $[0,1]^3$ with $40 \times 40 \times 40$ uniform grids involves $3$ millions of unknowns (d.o.f.) in the linear system, when a $\PP_1$ finite element is considered for the temperature.
For such a large size problem, memory limitation can rapidly arise with sequential algorithms.
...
DDM aims at dividing the computational domain in many subdomains on which we solve local problems with adequate interface conditions.
Two families of DDM exists: non-overlapping methods and overlapping methods such as the Schwarz method.

\noindent We use in this work a domain decomposition Schwarz method, enhanced by coarse space corrections through the \ff library \texttt{ffddm}.
...
It is well-known indeed that the main drawback of One-Level methods is their convergence rate that depends on the number of subdomains,
resulting in a poor scaling for large problems. 
This is mostly due to a lack of global communication between subdomains that exchange informations with their direct neighbors only.
The additional coarse space has consequently the task to spread the information to all subdomains at each iteration.

The data distribution among processors is done via an overlapping domain decomposition and involves a related linear algebra.
To enable parallel computing, the mesh is first split into subdomains using \texttt{Scotch} or \texttt{Metis} libraries. 
Fig. \ref{fig:mesh-partition} illustrates the domain decomposition of a cube of dimension $[0,1]^3$ into $8$ subdomains with \texttt{Metis} graph partitioner. 
...
\noindent The  final linear system of Eqs. \eqref{eq-newton-C1} - \eqref{eq-newton-C3} is solved in parallel using a GMRES Krylov method, with an Optimized Restricted Additive Schwarz (ORAS) preconditioner.  
...
We assess the strong scalability of the ORAS preconditioner on the 3D differentially heated cubic cavity.
...
Fig. \ref{fig-scalability} illustrates the evolution of the total wall clock time for different subdomains, and a good speed up is observed.
From $48$ to $320$ processors we observe a linear speed up. 
...
In Tab. \ref{tab-scalability} we detail the different runtimes relative to this test.
