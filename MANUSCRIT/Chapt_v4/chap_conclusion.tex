...
In this thesis, we have addressed the numerical simulations of natural convection with melting boundary problem.
...
\noindent The buoyancy-induced fluid motion in the liquid phase was modeled by the incompressible Navier-Stokes equations with Boussinesq approximation, and the phase-change was modeled by an enthalpy-porosity model using temperature-based
...
\noindent Concerning the numerical approach, the coupled momentum and energy equations was integrated in time using fully implicit second-order GEAR scheme, 
and the space discretization was based on a Taylor-Hood triangular finite elements, approximating the velocity and the pressure with $\PP_2$ and $\PP_1$ finite elements, respectively.
...
We have concentrated the grid refinement effort only in regions displaying high gradients of the computed variables, and prescribed coarser meshes in the solid phase and the stagnant region, reducing considerably the degrees of liberty by keeping high accuracy.
The accuracy of the numerical method was tested using manufactured solutions, and have exhibited as expected a second-order accurate method in both space and time.
...
In this manner, the user could implement easily similar problem with phase-change.
...
We started by simulating the natural convection of air and water inside differentially heated square enclosure and validated our results against classical benchmarks of classical convection.
Rayleigh numbers up to $10^8$, at the limit of the steadiness for the natural convection of air, were performed.
...
\noindent We also performed the natural convection of water case, which considers a non-linear variation of the density in the buoyancy force.
The comparison with existing numerical data in the literature exhibited very good agreement and the mesh adaptivity algorithm proved very efficient, allowing to capture accurately the anomalous variation of the density around $T = 4 \, ^oC$.
For all the simulations presented in the frame of natural convection problems, the steady state regimes were reached at most after $50$ CPU seconds of computations.

\noindent We then validated extensively the code by simulating the problem of melting and solidification of PCM.
We first considered the melting of n-octadecane and Gallium cases inside rectangular enclosures vertically heated to compare our simulation with available experimental and numerical results in the literature.
A mesh convergence test was addressed to ensure that we obtained a grid-converged solution.
The parameters of the adapted mesh, used for all simulations, exhibited a variation lower than $0.042 \%$ for the location of the solid-liquid interface and the time evolution of the Nusselt number.
...
The simulation of the melting of Gallium showed that our simulations lasts $245$ times faster when compared with the simulations of \cite{hannoun2003resolving} with a ratio of $185$ for the number of triangles.

\noindent We finally presented the challenging case of water freezing inside differentially heated cavity.
...
After a thorough investigation, we could conclude that our numerical method is validated by providing a good agreement with the benchmarks presented in the literature, for the natural convection of air and water and for the solid-liquid phase change problems including melting and solidification.
...
\noindent We first compared the behavior of the melting PCM when the latter is subject to heating from the sides or from below.
...
\noindent For the basal melting case, we developed a scale analysis during the linear regime and provided comprehensive description of the heat transfer processes during the melting.
Differences between lateral and basal melting were drawn, mainly the structure of the flow and the time evolution of the Nusselt number.
...
\noindent Further comprehensive investigation of the solidification process was also addressed by simulating alternate melting and solidification cycles.
...
However, when the discharge temperature was decreased by a factor of 5 the solidification and the melting occur over similar times.
The challenging task for the mesh adaptivity procedure was to track the two moving interfaces during the solidification cycles.

The third purpose of the thesis was to expand the numerical simulation to $3-$D configurations.
Parallel $3-$D tools for melting or solidification purpose involving natural convection flow, using domain decomposition method has been performed.
...
The linear system of equations resulting from the Newton linearization was solved using parallel \textit{GMRES} Krylov method
...
The influence of the three-dimensional effects in the flow, that we neglected during the $2-$D simulations, was highlighted by the $3-$D shape of the interface and the iso-surfaces, impacted by the secondary flow in the vicinity of the side walls.
...
One of the major limitation of the present physical model is that we have neglected the undercooling phenomenon, since we assume the temperature of fusion and the temperature of solidification equal for all simulations.
