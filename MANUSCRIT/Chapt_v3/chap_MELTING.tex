...
The purpose of this chapter is to validate accurately our developed method with respect to physical features of the problem.
...
Then, Secs. \ref{subsec-luo} and \ref{sec-solid-crust} show the capability of our code to deal with different geometrical shapes.
...
Although the non-linear definition of the body force, the striking feature of the water freezing simulation is the tracking of several interfaces, namely the solidification front and anomalous thermal variation of water density. 
...
%   \hline
%   \multicolumn{2}{c}{cylindrical with inner tubes} &  $5 \cdot 10^4$  & $0.2$   & $0.02$ & $10^{-2}$ & $\frac{\nu_l}{H}$ \\
%   \hline
%   \multicolumn{2}{c}{highly distorted mesh} &  $10^6 $  & $0.1$  & $4.854$ & $10^{-3}$ & $\frac{\nu_l}{H} \sqrt{\frac{Ra}{Pr}}$ \\
%   \hline
%   \multicolumn{2}{c}{water freezing} & $2.518084\cdot 10^{6}$ & $6.99$ & $0.125$ & $10^{-4}$ & $\frac{\nu_l}{H}$ \\
%\bottomrule
% \end{tabular}
...
\noindent The influence of $\CKC$ in the location of the front was shown by \cite{kheirabadi2015effect}.
...
\noindent \cite{Kumar2017} studied the effect of the mushy zone constant and its influence on the melt fraction, vortex strength and the amount of heat storage. They concluded that increasing $\CKC$ leads to a decrease of the convection strength and consequently leads to a decrease of the heat transfer rate. 
...
The mesh adaptation parameters that could influence the solution in our simulation are the interpolation error level \texttt{errh}, the minimum edge size \texttt{hmin} and the ratio for a prescribed smoothing of the metric \texttt{adaptratio}.
%The anisotropy of the mesh is a parameter of the algorithm and it was set to have value close to 1. 
We fixe the maximum and the minimum triangle size to avoid generating too large number of nodes, and we impose thus the anisotropy of the mesh to have value close to $1$.
...
In our simulation cases, Rayleigh numbers in the range of $10^6$ to $10^8$ and Prandtl number of order of $50$ lead to a dimensionless thickness of the thermal boundary layer of order of $\delta_\theta \sim 0.01$.
