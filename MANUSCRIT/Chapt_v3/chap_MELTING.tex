...
\noindent Then, Secs. \ref{subsec-luo} and \ref{sec-solid-crust} demonstrate the capability of our code to deal with different geometrical shapes.
...
The mesh adaptation strategy requires to set  the interpolation error level {\em errh}, the minimum and the maximum edge sizes {\em hmin} and {\em hmax}, and the {\em adaptratio} parameter, which defines the ratio for a prescribed smoothing of the metric.
The minimum size of triangles and the interpolation error level are set in order to capture both boundary layers region and phase-change interfaces.
...
The scale for boundary layers in natural convection flow along a vertical wall is given by Eqs. \ref{eq-scale-nbd-high-Pr} and \ref{eq-corr-Low-Pr}.
For high-Prandtl simulations, the thickness of the thermal boundary layer is thinner than the viscous boundary layers, and conversely for low-Prandtl fluid the viscous boundary layer is thinner than the thermal boundary layer.
...
A first usual approach is setting a very thin fluid layer of thickness $\delta x \sim 0.01$ with isothermal temperature in the vicinity of the hot wall (see \cite{dan-2014-JCP,Belhamadia2004_S}.
A second approach consist of setting an establishment regime, which will be applied in the current work.
...
The first consists of establishing first the steady state regime by solving the steady equation (see algorithm \ref{eq-weak-steady} ) and dropping then smoothly the temperature of the cold wall under the temperature of fusion.
The second consists of setting directly the cold temperature, which is more difficult case compared to the hot restart.
...
To ensure that we obtain a grid-converged solution, we first perform different mesh resolutions and test the influence of the number of triangles on the melting front, the Nusselt number, and the liquid fraction in Fig. \ref{fig:mesh-convergence-OKADA}.
Panels (a) to (c) 

We present in Fig. \ref{fig:pcm-valid} the computed solution for \textit{Benchmark $\#1$} case.
The experimental investigation of \cite{Okada1984} consists of a differentially heated square cavity of dimensions $1.5$ cm $\times \, 1.5$ cm.
...
\noindent Panel (a) shows the temperature distribution of the PCM at dimensionless time $t=78.7$ in the unit square domain $\Omega = (0,1)^2$. 
The enclosure is heated from the left and the melting expands therefore from the left to the right melting boundary (the blue color denotes the solid region).
...
More comprehensive descriptions of the influence of the convection flow on the interface will be further discussed in Chapter \ref{chap-MELTING-ANALYSIS}.

\noindent Panel (b) compares the location of the solid-liquid interface with experimental study of  \cite{Okada1984}.
...
In Fig. \ref{fig:pcm-valid}b, for the two time instants $t=39.9$ and $t=78.7$, the current work agrees well with the experimental results of \cite{Okada1984} at the bottom part of the melting front.
...
Numerical results for \textit{Benchmark $\#2$} are displayed in Fig. \ref{fig:pcm-Gong}.
...
Panel (a) illustrates the temperature distribution in the melting PCM, the streamlines, and the phase-change interface obtained by the present simulation.
...
\noindent The phase-change interface for four time steps, $t=0.25$,  $t=1$, $t=3$ and $t=5$ is represented in Fig. \ref{fig Bertran}.
...
\noindent The time evolution of the liquid fraction and the  Nusselt number are presented in Fig. \ref{fig-Nu-Lf-Bertran}.
...
The melting of gallium in a rectangular cavity was a controversial case since  \cite{dantzig1989modelling} raised the question 
...
To capture the very small cell during the first step of the melting, \cite{hannoun2003resolving} uses a $800 \times 1,120$ fixed grids in a rectangular domain of dimensions $6.35$ cm $\times \, 8.89$ cm. 
 \cite{hannoun2003resolving} have reported that the melting solution up to 32 s have required about $980$ CPU hours on a Compaq Alpha ($667$ MHz, ev$67$) processor.
However, a maximum of $4, 820$ triangles are necessary with our adaptive method, and only $4$ CPU hours are spent to reproduce the numerical result of \cite{hannoun2003resolving} up to the final time 280s, on an Intel ($2,8$ GHz, Core i$7$) processor.%, that is a ratio of $10^2$.
...
The time evolution of the flow  is presented in the Fig. \ref{fig-Gallium}. Temperature field, streamlines and position of the melting front are plotted for several time instants: $t= 0.0015$, $0.006$, $0.01$, and $0.019$.
...
Besides the complex fluid flow and the non-linear time evolution of the solid-liquid interface, the code have to handle the non-linear variation of the density presented in Eq. \ref{eq-dens-nonlin}.
Moreover, one can note that in addition to the non-linear formulation of $f_B(\theta)$ in Eq. \ref{eq-fBnonlin}, the coefficient $\Ray \times 1/(\beta \delta T)$ is very large since the Boussinesq approximation imposes a value of $\beta \delta T \ll 1$.
...
The melting of cylindrical PCM including inner heated pipes and the solidification of liquid PCM inside irregular cavity have also shown good agreement with existing numerical data.
