...
Having proven the efficiency and the accuracy of our numerical method to deal with the Navier-Stokes-Boussinesq equations, we now pay a closer attention to convective phase-change problems.
This chapter aims to validate accurately our developed method with respect to physical feature of the problem.
...
Although the non-linear definition of the body force, the striking feature of the water freezing case is the tracking of several interfaces, namely the solidification front and anomalous thermal variation of water density. % proving the effectiveness of the mesh adaptivity.
...
We present in this section the setting of some numerical parameters defined in Chap. \ref{chap-ToolBox}.
...
We simulate first the melting of an octadecane PCM within rectangular enclosure with the following dimensionless parameters: $\Ray = 3.27 \cdot 10^5$, $\Prd = 56.2$ and $\Ste = 0.045$.
...
The value of $\theta_{ck}$ is set in order to have the sharp variation of the derivative of d$L_f/$d$\theta$ near the new phase appearing in the system, liquid for melting ($\theta_{ck}=\varepsilon$) and solid  for freezing ($\theta_{ck}=\theta_f$) (see also \cite{dan-2014-JCP}).
...
We set accordingly for all subsequent simulations $\CKC= 10^6$ and $R_{ck} = \varepsilon$.
...
The scale for the thermal boundary layer ($\delta_T$) in natural convection flow along a vertical wall is given by \citep{bejan2013convection}:
...
\begin{figure}
	...
	$\Ray = 5 \cdot 10^4$, $\Prd =0.2$ and $\Ste = 0.02$.}  \label{fig-Luo-Field}
\end{figure}
...
\begin{figure} 
	\begin{center} 
		...
		\end{minipage}
	\end{center}
	\caption{Time evolution of the liquid fraction for one, four, and nine heated tubes. Comparison with numerical results of \cite{luo2015lattice}. $\Ray = 5 \cdot 10^4$, $\Prd =0.2$ and $\Ste = 0.02$.} \label{fig-Lf-Luo}
\end{figure}
...
\centering
\begin{tabular}{*{3}{c}}
   & {\small CPU time (s)} & {\small Number of triangles}  \\
  \toprule 
  case $\#1$ (Bench $\#1$) & $2876.39$ & $2,900$ \\
  case $\#1$ (Bench $\#2$)  & $40522.7$ & $7,000$ \\
  case $\#2$ & $2060.42$ & $3,076$ \\
  case $\#3$ & $14621.2$ & $4,820$ \\
  case $\#4$ & $6657.58$ & $2,769$ \\
