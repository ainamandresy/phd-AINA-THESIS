...
Panel (a) shows the temperature distribution in the PCM at dimensionless time $t=78.7$  (the blue color denotes the solid region) and panel (b) compare the location of the solid-liquid interface with experimental study of  \cite{Okada1984}.
...
\noindent In the experimental set up of, the author has reported that the top of the PCM was not perfectly insulated and consequently the growth of the experimental upper melting front was delayed.
In Fig. \ref{fig:pcm-valid}b, for the two time instants $t=39.9$ and $t=78.7$, the current work agrees well with the experimental results of \citep{Okada1984} at the bottom part of the melting front.
...
Panel (a) illustrates the temperature distribution in the melting PCM, the streamlines, and the phase-change interface.
...
\noindent We further inspect the melting front, the temporal evolution of the liquid fraction $L_f$ and the Nusselt number $N\!u$ at the left wall ($x=0$), for each of the five methods presented by \cite{bertrand1999melting}.
...
\noindent The time evolution of the Nusselt number and the liquid fraction are presented in Fig. \ref{fig-Nu-Lf-Bertran}.
...
Homogenous Dirichlet boundary conditions ($\vec u = 0$) are prescribed on the velocity over the whole $\partial \Omega$.

%We have compared for the investigated $\Ray$ number, the results when either the whole domain or the half of the domain is simulated, and the same numerical results were obtained.
%For the investigated $\Ray$ number, simulating the whole or the half of the domain exhibits the same results because of the axisymmetry of the problem.
%the simulation of the entire domain and the half of the domain and the same results were observed.
%However, the axisymmetry of the adapted mesh is not directly handle by the mesh adaptivity algorithm and could influence the numerical solution.
%To overcome this issue, the metrics for the mesh adaptivity is computed only on the half of the domain.

\noindent The physical parameters of the simulation are:  $\Ray = 5 \cdot 10^4$, $\Prd =0.2$ and $\Ste = 0.02$.
...
A reasonably close match with numerical results of \cite{luo2015lattice} is confirmed by looking at Fig. \ref{fig-Lf-Luo}a for the temporal evolution of the liquid fraction.
...
After achieving the convection steady pattern in the cavity (see Figure \ref{fig-T1w-isoT}), freezing starts by dropping progressively the temperature of the cold (right) wall from $T_c=\celsm{0}$ to  $T_c=\celsm{-10}$. 
...
The time evolution of the solid-liquid interface is depicted in Fig. \ref{fig-T2w-manip}d for several time instants from $t=0.05$ ($t_{\varphi}=0.05 [s]$) to final time $t=1.61$ ($2340 [s]$).
...
\begin{figure}
	...
	(c) finite-element mesh refined along the solid-liquid interface ($T = 0^oC$) and also along the line of maximum water density ($T = 4^oC$) (d) location of the solidification front for several time instants.}
	\label{fig-T2w-manip}
\end{figure}
%\begin{figure}
%	\begin{center}
