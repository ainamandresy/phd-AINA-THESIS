...
Fig. \ref{fig-Energ-cons}a illustrates the energy sources for electricity generation by seven region in the world in 2017.
...
The investigated octadecane PCM is heated from the right and melts from the right to the left.
...
These experimental investigations have been extensively used for numerical validation (see \cite{bertrand1999melting,gobin2000melting,wang2010numerical,dan-2014-JCP,rakotondrandisa2019numerical}) and have permitted a better comprehension of the physical phenomenon of the phase-change, mainly the heat transfer process.
The experimental results are very often formulated in the form of mathematical correlations.
\cite{Okada1984} have proposed for example correlations of the variation of dimensionless thermal energy stored as latent heat, and the average Nusselt number on the vertical wall with the dimensionless time.
Later, \cite{jany1988scaling} have formulated $N\!u$-$\Ray$ correlation through a scaling analysis and \cite{ho1984inward} have analyzed the solid-liquid interface position function of the configuration.
...
The previously mentioned investigations were mostly focused on studying separately melting or solidification problems and only recently for alternate melting and solidification complete cycles \citep{wang2010numerical}. 
However, cyclic, or periodic, melting and solidification problems have attracted considerable attention in the literature. 
\cite{ho1993periodic} and \cite{voller1996cyclic} studied numerically, periodic melting in a square enclosure. 
Recently, \cite{hosseini2014experimental} presented experimental studies for the melting and the solidification of a cylindrical PCM during charging and discharging process and \cite{chabot2017solid} studied analytically the effect of an alternate heating and cooling in a cylindrical PCM, with periodical boundary conditions.
...
%The usual approach could be divided into two main classes: melting from the side and melting from bellow.
%The first one could be representative of buildings applications, solar collectors or other thermal energy storages.
%The second one generally relates geophysical phenomenon or passive temperature control for electronic devices.
%Lateral melting of PCM have been largely studied experimentally, numerically, and theoretically, and could thus provided some reference cases for code validations.
%Basal melting have however attracted less attention.
...
%Since temperature gradients induce buoyancy forces in the liquid (melted) phase generating a significant convective flow, 
...
The method consists of solving the Navier-Stokes-Boussinesq equations by the mean of single domain approach using first order scheme.
Temperature gradients induce indeed buoyancy forces in the liquid (melted) phase and generate a significant convective flow.
...
A first objective of this thesis is to improve the existing code, developed by the numerical methods and applications group of the LMRS Laboratory, and to organize the program as a toolbox for the software FreeFem++.
...
The advantage of this formulation is to  permit a straightforward implementation of different types of non-linearities.
For the space discretization we use Taylor-Hood triangular finite elements, \ie P$_2$ for the velocity and temperature and P$_1$ for the pressure. 
...
Single domain methods require a refined mesh near the interface, where large enthalpy gradients have to be correctly resolved. 
...
There are three main novelties in the present numerical approach, when compared to  \cite{dan-2014-JCP}: 
\begin{enumerate}[label=(\roman*)]
...
\end{enumerate}
%{\em (i)} we use the Carman-Kozeny model to bring the velocity to zero inside the solid phase, instead of a  viscosity penalty method (imposing a large value of the viscosity in the solid), \\
...
%{\em (iii)} we improve the metric calculation procedure for mesh adaptivity.
