...
\subsection{Mesh adaptativity for melting}
...
At the same time, the minimum size of triangles should allow to capture the artificial mushy zone where discontinuous variables are smoothly regularized.
A minimum of 4 triangles should be included in the mushy zone to ensure an accurate location of the melting interface and a correct resolution of gradients.
For all subsequent simulations, the size of mushy zone is set to $\varepsilon = 0.02$, the minimum edge-length to $h_{min} = 10^{-3}$ and the maximum edge-lenght to $h_{max} = 0.1$.
